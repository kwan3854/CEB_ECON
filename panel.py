...
def folder_venv_callback(scene, context):
    ...
    if os.path.exists(directory):
        ...
        for name_file_dir in os.listdir(directory):
            if os.path.isdir(join(directory, name_file_dir)) and name_file_dir.endswith(
                "_39_env"
            ):
        ...
        for x in range(len(list_folder)):
            i = i + 1
            items.append(
                (list_folder[x], "%s" % list_folder[x], "Data: %s" % list_folder[x])
            )
        if len(items) == 0:
...
def update_bg_strength(self, context):
    bpy.data.worlds["Text2Light"].node_tree.nodes["Background"].inputs[
        1
    ].default_value = self.fl_bg_strength
...
class ECON_PT_generatation_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        rowC = layout.row(align=True).box()
        rowC = rowC.column(align=True)
        rowC.label(text="Generate Texture (Experimental)")
        rowC.prop(econ_prop, "stable_diffusion_prompt")
        rowC.operator("econ.execute_texture")
...
class ECON_PT_installation_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        flag_installed = join(
            ...
            econ_prop.str_custom_venv_name,
            "Lib",
            "site-packages",
            "torch",
        )
        ...
        if scene.ceb_econ_venv:
            ...
            _label_multiline(
                context=context, text="Full Path: " + path_venv, parent=row_subpath
            )
            ...
            if econ_prop.bool_custom_venv:
                ...
                row_subpath.prop(
                    econ_prop, "bool_custom_venv_name", text="Name", toggle=1
                )
                row_subpath.prop(
                    econ_prop, "bool_custom_venv_name_from_list", text="List", toggle=1
                )
                ...
                if econ_prop.bool_custom_venv_name_from_list:
                    row_sub_subpath = row.column(align=True)
                    # row_subpath = row_subpath.column(align=True)
                    row_sub_subpath.prop(
                        econ_prop, "enum_custom_venv_list", text="Folder"
                    )
            ...
            row.operator(
                "wm.url_open", text="Get/Install 7zip", icon="URL"
            ...
            if " " in path_venv:
                ...
                if econ_prop.bool_step1_create_econ_venv:
                    ...
                    row_sub_step1.operator(
                        "econ.enable_button"
                    ).button = "bool_step1_create_econ_venv"
                else:
            ...
            if econ_prop.bool_step2_downlad_econ:
                ...
                row_sub_step2.operator(
                    "econ.enable_button"
                ).button = "bool_step2_downlad_econ"
            else:
            ...
            if " " in path_venv:
                ...
                if econ_prop.bool_step3_install_econ_venv:
                    ...
                    row_sub_step3.operator(
                        "econ.enable_button"
                    ).button = "bool_step3_install_econ_venv"
                else:
            ...
            if econ_prop.bool_step4_install_compiler:
                ...
                row_sub_step4.operator(
                    "econ.enable_button"
                ).button = "bool_step4_install_compiler"
            else:
            ...
            if econ_prop.bool_step5_finish_econ:
                ...
                row_sub_step5.operator(
                    "econ.enable_button"
                ).button = "bool_step5_finish_econ"
            else:
            ...
            if econ_prop.bool_step6_install_pytorch:
                ...
                row_sub_step6.operator(
                    "econ.enable_button"
                ).button = "bool_step6_install_pytorch"
            else:
            ...
            row = layout.row(align=True).box()
            row = row.column(align=True)
            row.label(text="Install TEXTure")

            row_sub = row.row()
            row_step7 = row_sub.row()
            row_sub_step7 = row_sub.row()
            row_sub_step7.scale_x = 0.5
            if econ_prop.bool_step7_download_TEXTure:
                row_step7.enabled = False
                row_step7.operator("econ.download_texture")
                row_sub_step7.operator(
                    "econ.enable_button"
                ).button = "bool_step7_download_TEXTure"
            else:
                row_step7.enabled = True
                row_step7.operator("econ.download_texture")
            ...
            row.operator(
                "wm.url_open", text="Register", icon="URL"
            ...
            row.operator(
                "wm.url_open", text="Register on all ", icon="URL"
            ).url = "https://icon.is.tue.mpg.de/user.php"
...
class ECONMySettings(PropertyGroup):
    ...
    str_venv_path: StringProperty(
        name="Venv Path", description="Place to install/use Venv", default=path_txt
    )
    str_sd_prompt: StringProperty(name="Prompt", description="Prompt for text2light")
    str_custom_venv_name: StringProperty(
        name="Name", default=DEFAULT_NAME, update=updt_custom_venv_name
    )
    bool_custom_venv: BoolProperty(
        name="Custom venv", default=False, update=updt_bool_custom_venv
    )
    str_custom_venv_name: StringProperty(
        name="Name", default=DEFAULT_NAME, update=updt_custom_venv_name
    )
    bool_custom_venv_name: BoolProperty(
        name="Custom venv Name", default=False, update=updt_bool_venv_custom_name
    )
    bool_custom_venv_name_from_list: BoolProperty(
        name="Custom venv Name from List",
        default=False,
        update=updt_bool_venv_custom_name_from_list,
    )
    ...
    fl_bg_strength: FloatProperty(
        name="Strength", min=0.0, default=1.0, update=update_bg_strength
    )
    str_smpl_username: StringProperty(name="user")
    str_smpl_password: StringProperty(name="pass", subtype="PASSWORD")
    bool_erase_zip: BoolProperty(
        name="Erase Zip", description="Erase zip after unzipping", default=False
    )
    ...
    bool_step7_download_TEXTure: BoolProperty(name="Step 7", default=False)
